...
#include <trace/events/cpufreq_interactiveS9.h>

#define CONFIG_DYNAMIC_MODE_SUPPORT
//#define CONFIG_DYNAMIC_MODE_SUPPORT_DEBUG

struct cpufreq_interactiveS9_cpuinfo {
...
struct cpufreq_interactiveS9_tunables {
	int usage_count;
...
static struct cpufreq_interactiveS9_tunables *common_tunables;
static struct cpufreq_interactiveS9_tunables *tuned_parameters[NR_CPUS] = {NULL, };

static struct attribute_group *get_sysfs_attr(void);

static void cpufreq_interactiveS9_timer_resched(
	struct cpufreq_interactiveS9_cpuinfo *pcpu)
{
	struct cpufreq_interactiveS9_tunables *tunables =
 ...
 * function.
 */
static void cpufreq_interactiveS9_timer_start(
	struct cpufreq_interactiveS9_tunables *tunables, int cpu)
{
	struct cpufreq_interactiveS9_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
...
static unsigned int freq_to_targetload(
	struct cpufreq_interactiveS9_tunables *tunables, unsigned int freq)
{
	int i;
	unsigned int ret;
	unsigned long flags;
 ...
 */
static unsigned int choose_freq(struct cpufreq_interactiveS9_cpuinfo *pcpu,
...
static void enter_mode(struct cpufreq_interactiveS9_tunables * tunables)
{
...
static void exit_mode(struct cpufreq_interactiveS9_tunables * tunables)
{
...
static void cpufreq_interactiveS9_timer(unsigned long data)
{
#ifdef CONFIG_EXYNOS_WD_DVFS
	u64 cpki;
	u64 core = 1;
	...
	if (pcpu->policy->cur >= tunables->hispeed_freq &&
		...
		trace_cpufreq_interactiveS9_notyet(
	...
	if (pcpu->target_freq == new_freq &&
			pcpu->target_freq <= pcpu->policy->cur) {
		trace_cpufreq_interactiveS9_already(
			data, cpu_load, pcpu->target_freq,
			pcpu->policy->cur, new_freq);
		spin_unlock_irqrestore(&pcpu->target_freq_lock, flags);
		goto rearm;
	...
	trace_cpufreq_interactiveS9_target(data, cpu_load, pcpu->target_freq,
					 pcpu->policy->cur, new_freq);
	...
	wake_up_process(speedchange_task2);
...
rearm:
	if (!timer_pending(&pcpu->cpu_timer))
		cpufreq_interactiveS9_timer_resched(pcpu);
...
static void cpufreq_interactiveS9_idle_end(void)
{
	struct cpufreq_interactiveS9_cpuinfo *pcpu =
	...
	if (!timer_pending(&pcpu->cpu_timer)) {
		...
		cpufreq_interactiveS9_timer(smp_processor_id());
	}

	up_read(&pcpu->enable_sem);
}

static void cpufreq_interactiveS9_get_policy_info(struct cpufreq_policy *policy,
...
static void cpufreq_interactiveS9_adjust_cpu(unsigned int cpu,
					   struct cpufreq_policy *policy)
{
	struct cpufreq_interactiveS9_cpuinfo *pcpu;
	u64 hvt, fvt;
	...
	cpufreq_interactiveS9_get_policy_info(policy, &max_freq, &hvt, &fvt);
	...
	if (max_freq != policy->cur) {
		__cpufreq_driver_target(policy, max_freq, CPUFREQ_RELATION_C); /* C, L, H */
		for_each_cpu(i, policy->cpus) {
			pcpu = &per_cpu(cpuinfo, i);
	...
	trace_cpufreq_interactiveS9_setspeed(cpu, max_freq, policy->cur);
}

static int cpufreq_interactiveS9_speedchange_task2(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			if (likely(down_read_trylock(&pcpu->enable_sem))) {
				if (likely(pcpu->governor_enabled))
					cpufreq_interactiveS9_adjust_cpu(cpu,
							pcpu->policy);
				up_read(&pcpu->enable_sem);
			}
...
static void cpufreq_interactiveS9_boost(struct cpufreq_interactiveS9_tunables *tunables)
{
	int i;
	int anyboost = 0;
	...
	if (anyboost && speedchange_task2)
		wake_up_process(speedchange_task2);
}

static int cpufreq_interactiveS9_notifier(
	struct notifier_block *nb, unsigned long val, void *data)
{
	...
	if (val == CPUFREQ_PRECHANGE) {
		...
		for_each_cpu(cpu, pcpu->policy->cpus) {
			struct cpufreq_interactiveS9_cpuinfo *pjcpu =
				&per_cpu(cpuinfo, cpu);
			if (cpu != freq->cpu) {
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_interactiveS9_notifier,
};
...
static ssize_t show_above_hispeed_delay(
	struct cpufreq_interactiveS9_tunables *tunables, char *buf)
{
	int i;
	ssize_t ret = 0;
	unsigned long flags;
...
static ssize_t show_hispeed_freq(struct cpufreq_interactiveS9_tunables *tunables,
		char *buf)
{
#ifdef CONFIG_DYNAMIC_MODE_SUPPORT
...
static ssize_t store_hispeed_freq(struct cpufreq_interactiveS9_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
...
static ssize_t show_go_hispeed_load(struct cpufreq_interactiveS9_tunables
		*tunables, char *buf)
{
#ifdef CONFIG_DYNAMIC_MODE_SUPPORT
...
static ssize_t store_go_hispeed_load(struct cpufreq_interactiveS9_tunables
...
#ifdef CONFIG_EXYNOS_WD_DVFS
static ssize_t show_wd_boundary(struct cpufreq_interactiveS9_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%u\n", tunables->wd_boundary);
}

static ssize_t store_wd_boundary(struct cpufreq_interactiveS9_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_min_sample_time(struct cpufreq_interactiveS9_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->min_sample_time);
}

static ssize_t store_min_sample_time(struct cpufreq_interactiveS9_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_timer_rate(struct cpufreq_interactiveS9_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%lu\n", tunables->timer_rate);
}

static ssize_t store_timer_rate(struct cpufreq_interactiveS9_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
...
static ssize_t show_timer_slack(struct cpufreq_interactiveS9_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%d\n", tunables->timer_slack_val);
}

static ssize_t store_timer_slack(struct cpufreq_interactiveS9_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_boost(struct cpufreq_interactiveS9_tunables *tunables,
			  char *buf)
{
	return sprintf(buf, "%d\n", tunables->boost_val);
}

static ssize_t store_boost(struct cpufreq_interactiveS9_tunables *tunables,
			   const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (tunables->boost_val) {
		trace_cpufreq_interactiveS9_boost("on");
		if (!tunables->boosted)
			cpufreq_interactiveS9_boost(tunables);
	} else {
		tunables->boostpulse_endtime = ktime_to_us(ktime_get());
		trace_cpufreq_interactiveS9_unboost("off");
	}

	return count;
}

static ssize_t store_boostpulse(struct cpufreq_interactiveS9_tunables *tunables,
				const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	trace_cpufreq_interactiveS9_boost("pulse");
	if (!tunables->boosted)
		cpufreq_interactiveS9_boost(tunables);
	return count;
}

static ssize_t show_boostpulse_duration(struct cpufreq_interactiveS9_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%d\n", tunables->boostpulse_duration_val);
}

static ssize_t store_boostpulse_duration(struct cpufreq_interactiveS9_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_io_is_busy(struct cpufreq_interactiveS9_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->io_is_busy);
}

static ssize_t store_io_is_busy(struct cpufreq_interactiveS9_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
#ifdef CONFIG_DYNAMIC_MODE_SUPPORT
static ssize_t show_mode(struct cpufreq_interactiveS9_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%u\n", tunables->mode);
}

static ssize_t store_mode(struct cpufreq_interactiveS9_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	long unsigned int val;
...
static ssize_t show_param_index(struct cpufreq_interactiveS9_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%u\n", tunables->param_index);
}

static ssize_t store_param_index(struct cpufreq_interactiveS9_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	long unsigned int val;
...
static struct attribute *interactiveS9_attributes_gov_sys[] = {
...
static struct attribute_group interactiveS9_attr_group_gov_sys = {
	.attrs = interactiveS9_attributes_gov_sys,
	.name = "interactiveS9",
};

/* Per policy governor instance */
static struct attribute *interactiveS9_attributes_gov_pol[] = {
...
static struct attribute_group interactiveS9_attr_group_gov_pol = {
	.attrs = interactiveS9_attributes_gov_pol,
	.name = "interactiveS9",
};
...
{
	if (have_governor_per_policy())
		return &interactiveS9_attr_group_gov_pol;
	else
		return &interactiveS9_attr_group_gov_sys;
}

static int cpufreq_interactiveS9_idle_notifier(struct notifier_block *nb,
					     unsigned long val,
					     void *data)
{
	if (val == IDLE_END)
		cpufreq_interactiveS9_idle_end();

	return 0;
}

static struct notifier_block cpufreq_interactiveS9_idle_nb = {
	.notifier_call = cpufreq_interactiveS9_idle_notifier,
};

#ifdef CONFIG_DYNAMIC_MODE_SUPPORT
static void cpufreq_param_set_init(struct cpufreq_interactiveS9_tunables *tunables)
{
	unsigned int i;
...
static int cpufreq_governor_interactiveS9(struct cpufreq_policy *policy,
		unsigned int event)
{
	int rc;
	unsigned int j;
	...
	case CPUFREQ_GOV_POLICY_INIT:
		...
		if (!policy->governor->initialized) {
			idle_notifier_register(&cpufreq_interactiveS9_idle_nb);
#ifdef CONFIG_EXYNOS_WD_DVFS
		...
		if (!--tunables->usage_count) {
			if (policy->governor->initialized == 1) {
				...
				cpufreq_unregister_notifier(&cpufreq_notifier_block,
						CPUFREQ_TRANSITION_NOTIFIER);
				idle_notifier_unregister(&cpufreq_interactiveS9_idle_nb);
			}
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			cpufreq_interactiveS9_timer_start(tunables, j);
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_INTERACTIVES9
static
#endif
struct cpufreq_governor cpufreq_gov_interactiveS9 = {
	.name = "interactiveS9",
	.governor = cpufreq_governor_interactiveS9,
	.max_transition_latency = 10000000,
	.owner = THIS_MODULE,
};

static void cpufreq_interactiveS9_nop_timer(unsigned long data)
{
}

unsigned int cpufreq_interactiveS9_get_hispeed_freq(int cpu)
{
	struct cpufreq_interactiveS9_cpuinfo *pcpu =
			&per_cpu(cpuinfo, cpu);
	struct cpufreq_interactiveS9_tunables *tunables;
...
static int cpufreq_interactiveS9_cluster1_min_qos_handler(struct notifier_block *b,
						unsigned long val, void *v)
{
	...
	trace_cpufreq_interactiveS9_cpu_min_qos(cpu, val, pcpu->policy->cur);
...
static struct notifier_block cpufreq_interactiveS9_cluster1_min_qos_notifier = {
	.notifier_call = cpufreq_interactiveS9_cluster1_min_qos_handler,
};

static int cpufreq_interactiveS9_cluster1_max_qos_handler(struct notifier_block *b,
						unsigned long val, void *v)
{
	struct cpufreq_interactiveS9_cpuinfo *pcpu;
	struct cpufreq_interactiveS9_tunables *tunables;
	...
	trace_cpufreq_interactiveS9_cpu_max_qos(cpu, val, pcpu->policy->cur);
...
static struct notifier_block cpufreq_interactiveS9_cluster1_max_qos_notifier = {
	.notifier_call = cpufreq_interactiveS9_cluster1_max_qos_handler,
};

static int cpufreq_interactiveS9_cluster0_min_qos_handler(struct notifier_block *b,
						unsigned long val, void *v)
{
	struct cpufreq_interactiveS9_cpuinfo *pcpu;
	struct cpufreq_interactiveS9_tunables *tunables;
...
static struct notifier_block cpufreq_interactiveS9_cluster0_min_qos_notifier = {
	.notifier_call = cpufreq_interactiveS9_cluster0_min_qos_handler,
};

static int cpufreq_interactiveS9_cluster0_max_qos_handler(struct notifier_block *b,
						unsigned long val, void *v)
{
	struct cpufreq_interactiveS9_cpuinfo *pcpu;
	struct cpufreq_interactiveS9_tunables *tunables;
...
static struct notifier_block cpufreq_interactiveS9_cluster0_max_qos_notifier = {
	.notifier_call = cpufreq_interactiveS9_cluster0_max_qos_handler,
};
#endif

static int __init cpufreq_interactiveS9_init(void)
{
	unsigned int i;
	struct cpufreq_interactiveS9_cpuinfo *pcpu;
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_interactiveS9_timer;
		pcpu->cpu_timer.data = i;
		init_timer(&pcpu->cpu_slack_timer);
		pcpu->cpu_slack_timer.function = cpufreq_interactiveS9_nop_timer;
	...
	speedchange_task2 =
		kthread_create(cpufreq_interactiveS9_speedchange_task2, NULL,
				"cfinteractiveS9");
	if (IS_ERR(speedchange_task2))
	...
	pm_qos_add_notifier(PM_QOS_CLUSTER1_FREQ_MAX, &cpufreq_interactiveS9_cluster1_max_qos_notifier);
	pm_qos_add_notifier(PM_QOS_CLUSTER0_FREQ_MIN, &cpufreq_interactiveS9_cluster0_min_qos_notifier);
	pm_qos_add_notifier(PM_QOS_CLUSTER0_FREQ_MAX, &cpufreq_interactiveS9_cluster0_max_qos_notifier);
#endif

	return cpufreq_register_governor(&cpufreq_gov_interactiveS9);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_INTERACTIVES9
fs_initcall(cpufreq_interactiveS9_init);
#else
module_init(cpufreq_interactiveS9_init);
#endif
...
MODULE_DESCRIPTION("'cpufreq_interactiveS9' - A cpufreq governor for "
