/*
 * drivers/cpufreq/cpufreq_thunderstorm2.c
 *
...
#include <trace/events/cpufreq_thunderstorm2.h>
...
struct cpufreq_thunderstorm2_tunables {
	int usage_count;
...
static struct cpufreq_thunderstorm2_tunables *common_tunables;
static struct cpufreq_thunderstorm2_tunables *tuned_parameters[NR_CPUS] = {NULL, };

static struct attribute_group *get_sysfs_attr(void);

static void cpufreq_thunderstorm2_timer_resched(
 ...
 * function.
 */
static void cpufreq_thunderstorm2_timer_start(
	struct cpufreq_thunderstorm2_tunables *tunables, int cpu)
{
	struct cpufreq_thunderstorm2_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
...
static unsigned int freq_to_targetload(
	struct cpufreq_thunderstorm2_tunables *tunables, unsigned int freq)
{
	int i;
	unsigned int ret;
	unsigned long flags;
 ...
 */
static unsigned int choose_freq(struct cpufreq_thunderstorm2_cpuinfo *pcpu,
...
static void enter_mode(struct cpufreq_thunderstorm2_tunables * tunables)
{
...
static void exit_mode(struct cpufreq_thunderstorm2_tunables * tunables)
{
...
static void cpufreq_thunderstorm2_timer(unsigned long data)
{
#ifdef CONFIG_EXYNOS_WD_DVFS
	u64 cpki;
	u64 core = 1;
	...
	/* fix for not editable timer_rate - before was alyways as prev_timer_rate */
	if (screen_on
		&& tunables->timer_rate != tunables->prev_timer_rate)
		tunables->timer_rate = tunables->timer_rate;
	else if (!screen_on
	...
	if (pcpu->policy->cur >= tunables->hispeed_freq &&
		...
		trace_cpufreq_thunderstorm2_notyet(
	...
	if (pcpu->target_freq == new_freq &&
			pcpu->target_freq <= pcpu->policy->cur) {
		trace_cpufreq_thunderstorm2_already(
			data, cpu_load, pcpu->target_freq,
			pcpu->policy->cur, new_freq);
		spin_unlock_irqrestore(&pcpu->target_freq_lock, flags);
		goto rearm;
	...
	trace_cpufreq_thunderstorm2_target(data, cpu_load, pcpu->target_freq,
					 pcpu->policy->cur, new_freq);
	...
	wake_up_process(speedchange_task3);
...
rearm:
	if (!timer_pending(&pcpu->cpu_timer))
		cpufreq_thunderstorm2_timer_resched(pcpu);
...
static void cpufreq_thunderstorm2_idle_end(void)
{
	struct cpufreq_thunderstorm2_cpuinfo *pcpu =
	...
	if (!timer_pending(&pcpu->cpu_timer)) {
		...
		cpufreq_thunderstorm2_timer(smp_processor_id());
	}

	up_read(&pcpu->enable_sem);
}

static void cpufreq_thunderstorm2_get_policy_info(struct cpufreq_policy *policy,
					   ...
					   struct cpufreq_policy *policy)
{
	...
	cpufreq_thunderstorm2_get_policy_info(policy, &max_freq, &hvt, &fvt);
	...
	trace_cpufreq_thunderstorm2_setspeed(cpu, max_freq, policy->cur);
}

static int cpufreq_thunderstorm2_speedchange_task3(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			if (likely(down_read_trylock(&pcpu->enable_sem))) {
				if (likely(pcpu->governor_enabled))
					cpufreq_thunderstorm2_adjust_cpu(cpu,
							pcpu->policy);
				up_read(&pcpu->enable_sem);
			}
...
static void cpufreq_thunderstorm2_boost(struct cpufreq_thunderstorm2_tunables *tunables)
{
	int i;
	int anyboost = 0;
	...
	if (anyboost && speedchange_task3)
		wake_up_process(speedchange_task3);
}

static int cpufreq_thunderstorm2_notifier(
	struct notifier_block *nb, unsigned long val, void *data)
{
	...
	if (val == CPUFREQ_PRECHANGE) {
		...
		for_each_cpu(cpu, pcpu->policy->cpus) {
			struct cpufreq_thunderstorm2_cpuinfo *pjcpu =
				&per_cpu(cpuinfo, cpu);
			if (cpu != freq->cpu) {
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_thunderstorm2_notifier,
};
...
static ssize_t show_hispeed_freq(struct cpufreq_thunderstorm2_tunables *tunables,
		char *buf)
{
#ifdef CONFIG_DYNAMIC_MODE_SUPPORT
...
static ssize_t store_hispeed_freq(struct cpufreq_thunderstorm2_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
...
static ssize_t store_go_hispeed_load(struct cpufreq_thunderstorm2_tunables
...
#ifdef CONFIG_EXYNOS_WD_DVFS
static ssize_t show_wd_boundary(struct cpufreq_thunderstorm2_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%u\n", tunables->wd_boundary);
}

static ssize_t store_wd_boundary(struct cpufreq_thunderstorm2_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_min_sample_time(struct cpufreq_thunderstorm2_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->min_sample_time);
}

static ssize_t store_min_sample_time(struct cpufreq_thunderstorm2_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_timer_rate(struct cpufreq_thunderstorm2_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%lu\n", tunables->timer_rate);
}

static ssize_t store_timer_rate(struct cpufreq_thunderstorm2_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
...
static ssize_t show_screen_off_max(struct cpufreq_thunderstorm2_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%lu\n", tunables->screen_off_max);
}

static ssize_t store_screen_off_max(struct cpufreq_thunderstorm2_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_screen_off_timer_rate(struct cpufreq_thunderstorm2_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%lu\n", tunables->screen_off_timer_rate);
}

static ssize_t store_screen_off_timer_rate(struct cpufreq_thunderstorm2_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val, val_round;
	...
	val_round = jiffies_to_usecs(usecs_to_jiffies(val));
	if (val != val_round)
		pr_warn("screen_off_timer_rate not aligned to jiffy. Rounded up to %lu\n",
			val_round);

	tunables->screen_off_timer_rate = val_round;
	return count;
}
...
static ssize_t store_down_low_load_threshold(struct cpufreq_thunderstorm2_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val, val_round;
...
static ssize_t store_timer_slack(struct cpufreq_thunderstorm2_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_boost(struct cpufreq_thunderstorm2_tunables *tunables,
			  char *buf)
{
	return sprintf(buf, "%d\n", tunables->boost_val);
}

static ssize_t store_boost(struct cpufreq_thunderstorm2_tunables *tunables,
			   const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (tunables->boost_val) {
		trace_cpufreq_thunderstorm2_boost("on");
		if (!tunables->boosted)
			cpufreq_thunderstorm2_boost(tunables);
	} else {
		tunables->boostpulse_endtime = ktime_to_us(ktime_get());
		trace_cpufreq_thunderstorm2_unboost("off");
	}

	return count;
}

static ssize_t store_boostpulse(struct cpufreq_thunderstorm2_tunables *tunables,
				const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	trace_cpufreq_thunderstorm2_boost("pulse");
	if (!tunables->boosted)
		cpufreq_thunderstorm2_boost(tunables);
	return count;
}

static ssize_t show_boostpulse_duration(struct cpufreq_thunderstorm2_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%d\n", tunables->boostpulse_duration_val);
}

static ssize_t store_boostpulse_duration(struct cpufreq_thunderstorm2_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_io_is_busy(struct cpufreq_thunderstorm2_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->io_is_busy);
}

static ssize_t store_io_is_busy(struct cpufreq_thunderstorm2_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
#ifdef CONFIG_DYNAMIC_MODE_SUPPORT
static ssize_t show_mode(struct cpufreq_thunderstorm2_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%u\n", tunables->mode);
}

static ssize_t store_mode(struct cpufreq_thunderstorm2_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	long unsigned int val;
...
static ssize_t show_param_index(struct cpufreq_thunderstorm2_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%u\n", tunables->param_index);
}

static ssize_t store_param_index(struct cpufreq_thunderstorm2_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	long unsigned int val;
...
static struct attribute *thunderstorm2_attributes_gov_sys[] = {
...
static struct attribute_group thunderstorm2_attr_group_gov_sys = {
	.attrs = thunderstorm2_attributes_gov_sys,
	.name = "thunderstorm2",
};

/* Per policy governor instance */
static struct attribute *thunderstorm2_attributes_gov_pol[] = {
...
static struct attribute_group thunderstorm2_attr_group_gov_pol = {
	.attrs = thunderstorm2_attributes_gov_pol,
	.name = "thunderstorm2",
};
...
{
	if (have_governor_per_policy())
		return &thunderstorm2_attr_group_gov_pol;
	else
		return &thunderstorm2_attr_group_gov_sys;
}

static int cpufreq_thunderstorm2_idle_notifier(struct notifier_block *nb,
					     unsigned long val,
					     void *data)
{
	if (val == IDLE_END)
		cpufreq_thunderstorm2_idle_end();

	return 0;
}

static struct notifier_block cpufreq_thunderstorm2_idle_nb = {
	.notifier_call = cpufreq_thunderstorm2_idle_notifier,
};

#ifdef CONFIG_DYNAMIC_MODE_SUPPORT
static void cpufreq_param_set_init(struct cpufreq_thunderstorm2_tunables *tunables)
{
	unsigned int i;
...
static int cpufreq_governor_thunderstorm2(struct cpufreq_policy *policy,
		unsigned int event)
{
	int rc;
	unsigned int j;
	...
	case CPUFREQ_GOV_POLICY_INIT:
		...
		if (!tuned_parameters[policy->cpu]) {
			...
			tunables->screen_off_timer_rate = SCREEN_OFF_TIMER_RATE;
		...
		if (!policy->governor->initialized) {
			idle_notifier_register(&cpufreq_thunderstorm2_idle_nb);
#ifdef CONFIG_EXYNOS_WD_DVFS
		...
		if (!--tunables->usage_count) {
			if (policy->governor->initialized == 1) {
				...
				cpufreq_unregister_notifier(&cpufreq_notifier_block,
						CPUFREQ_TRANSITION_NOTIFIER);
				idle_notifier_unregister(&cpufreq_thunderstorm2_idle_nb);
			}
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			cpufreq_thunderstorm2_timer_start(tunables, j);
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_THUNDERSTORM2
static
#endif
struct cpufreq_governor cpufreq_gov_thunderstorm2 = {
	.name = "thunderstorm2",
	.governor = cpufreq_governor_thunderstorm2,
	.max_transition_latency = 10000000,
	.owner = THIS_MODULE,
};

static void cpufreq_thunderstorm2_nop_timer(unsigned long data)
{
}

unsigned int cpufreq_thunderstorm2_get_hispeed_freq(int cpu)
{
	struct cpufreq_thunderstorm2_cpuinfo *pcpu =
			&per_cpu(cpuinfo, cpu);
	struct cpufreq_thunderstorm2_tunables *tunables;
...
static int cpufreq_thunderstorm2_cluster1_min_qos_handler(struct notifier_block *b,
						unsigned long val, void *v)
{
	...
	trace_cpufreq_thunderstorm2_cpu_min_qos(cpu, val, pcpu->policy->cur);
...
static struct notifier_block cpufreq_thunderstorm2_cluster1_min_qos_notifier = {
	.notifier_call = cpufreq_thunderstorm2_cluster1_min_qos_handler,
};

static int cpufreq_thunderstorm2_cluster1_max_qos_handler(struct notifier_block *b,
						unsigned long val, void *v)
{
	struct cpufreq_thunderstorm2_cpuinfo *pcpu;
	struct cpufreq_thunderstorm2_tunables *tunables;
	...
	trace_cpufreq_thunderstorm2_cpu_max_qos(cpu, val, pcpu->policy->cur);
...
static struct notifier_block cpufreq_thunderstorm2_cluster1_max_qos_notifier = {
	.notifier_call = cpufreq_thunderstorm2_cluster1_max_qos_handler,
};

static int cpufreq_thunderstorm2_cluster0_min_qos_handler(struct notifier_block *b,
						unsigned long val, void *v)
{
	struct cpufreq_thunderstorm2_cpuinfo *pcpu;
	struct cpufreq_thunderstorm2_tunables *tunables;
...
static struct notifier_block cpufreq_thunderstorm2_cluster0_min_qos_notifier = {
	.notifier_call = cpufreq_thunderstorm2_cluster0_min_qos_handler,
};

static int cpufreq_thunderstorm2_cluster0_max_qos_handler(struct notifier_block *b,
						unsigned long val, void *v)
{
	struct cpufreq_thunderstorm2_cpuinfo *pcpu;
	struct cpufreq_thunderstorm2_tunables *tunables;
...
static struct notifier_block cpufreq_thunderstorm2_cluster0_max_qos_notifier = {
	.notifier_call = cpufreq_thunderstorm2_cluster0_max_qos_handler,
};
#endif

static int __init cpufreq_thunderstorm2_init(void)
{
	unsigned int i;
	struct cpufreq_thunderstorm2_cpuinfo *pcpu;
	screen_on = true;
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_thunderstorm2_timer;
		pcpu->cpu_timer.data = i;
		init_timer(&pcpu->cpu_slack_timer);
		pcpu->cpu_slack_timer.function = cpufreq_thunderstorm2_nop_timer;
	...
	speedchange_task3 =
		kthread_create(cpufreq_thunderstorm2_speedchange_task3, NULL,
				"cfthunderstorm2");
	if (IS_ERR(speedchange_task3))
	...
	pm_qos_add_notifier(PM_QOS_CLUSTER1_FREQ_MAX, &cpufreq_thunderstorm2_cluster1_max_qos_notifier);
	pm_qos_add_notifier(PM_QOS_CLUSTER0_FREQ_MIN, &cpufreq_thunderstorm2_cluster0_min_qos_notifier);
	pm_qos_add_notifier(PM_QOS_CLUSTER0_FREQ_MAX, &cpufreq_thunderstorm2_cluster0_max_qos_notifier);
#endif

	return cpufreq_register_governor(&cpufreq_gov_thunderstorm2);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_THUNDERSTORM2
fs_initcall(cpufreq_thunderstorm2_init);
#else
module_init(cpufreq_thunderstorm2_init);
#endif
...
MODULE_DESCRIPTION("'cpufreq_thunderstorm2' - A cpufreq governor for "
